...
/**
 * Validate collection creation fields
 */
export function validateCollectionCreate(data: unknown): ValidationError[] {
...
/**
 * Validate collection update fields
 */
export function validateCollectionUpdate(data: unknown): ValidationError[] {
  const errors: ValidationError[] = []
  ...
  // Check if galleryId is being modified
  if ('galleryId' in body) {
    errors.push({ field: 'galleryId', message: 'Gallery cannot be changed' })
  }
  ...
  if (body.heroImageUrl !== undefined && body.heroImageUrl !== null) {
    if (typeof body.heroImageUrl !== 'string') {
      errors.push({ field: 'heroImageUrl', message: 'Hero image URL must be a string' })
    }
  }
