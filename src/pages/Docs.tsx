...
import { ChevronRight, Clock, BookOpen, User, AlertCircle } from 'lucide-react';
import { gettingStartedDocs, accountDocs, galleryTroubleshootingDoc, type DocSection } from '../data/docs';

type TabId = 'getting-started' | 'account' | 'troubleshooting';
...
export function Docs() {
  ...
  const [activeSection, setActiveSection] = useState<string>(gettingStartedDocs[0].id);
  ...
  const getCurrentDocs = (): DocSection[] => {
    switch (activeTab) {
      ...
      case 'troubleshooting':
        return [galleryTroubleshootingDoc];
    }
  };
  ...
  const handleTabChange = (tabId: TabId) => {
    setActiveTab(tabId);
    const docs = tabId === 'getting-started' ? gettingStartedDocs : tabId === 'account' ? accountDocs : [galleryTroubleshootingDoc];
      ...
      <div className="bg-white border-b border-canvas-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-canvas-600">
            Learn how to use VFA Gallery to showcase your artwork.
          </p>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                    activeTab === tab.id
                      ? 'border-artist-500 text-artist-600'
            ...
            <nav className="lg:sticky lg:top-36 space-y-1">
              {currentDocs.map((doc, index) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 text-sm text-left rounded-lg transition-colors ${
                    activeSection === doc.id
                      ? 'bg-artist-100 text-artist-700 font-medium'
                ...
                >
                  ...
                  {activeTab !== 'getting-started' && (
                    <ChevronRight className={`w-4 h-4 ${activeSection === doc.id ? 'text-artist-500' : 'text-canvas-400'}`} />
                  )}
            ...
            <article className="bg-white rounded-xl border border-canvas-200 shadow-sm p-6 sm:p-8">
              ...
              <div
                className="prose prose-canvas max-w-none
                  ...
                  prose-a:text-artist-600 prose-a:no-underline hover:prose-a:underline
                      ...
                      <>
                        {prevDoc ? (
                          <button
                            ...
                            className="text-sm text-canvas-600 hover:text-artist-600 transition-colors"
                          >
                            ← {prevDoc.title}
                          </button>
                        ) : <span />}
                        {nextDoc && (
                          <button
                            ...
                            className="text-sm font-medium text-artist-600 hover:text-artist-700 transition-colors"
                          >
                            {nextDoc.title} →
                          </button>
                        )}
